...
export function ArticleHeader({ article }: { article: Article }) {
  return (
    <>
      <h2 style={{ margin: 0, color: theme.colors.textPrimary, fontSize: theme.typography.size.xl }}>
        {article.title}
      </h2>
      <p
        style={{
    ...
    <div>
      {article.body.map((block, index) => {
        switch (block.type) {
          case "paragraph":
            return (
              <p key={index} style={{ marginTop: theme.spacing.xl, color: theme.colors.textMuted }}>
